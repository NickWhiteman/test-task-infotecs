...
const body = document.querySelector(".body");/* боди для затемнения экрана в режиме OpenModal */
...
const sortAboutLength = () => {
  const sortData = [];
  let aboutCollection = data.map((item) => item.about);
  aboutCollection.sort((a, b) => a.about < b.about ? 1 : -1);
  aboutCollection.forEach((about) => {
    data.forEach((item) =>
      about === item.about && sortData.push(item))}
  );
...
/*
  Функция deleteEditWindow Занимается тем что удаляет окно редактирования данных строки, в случае если существует. Несет роль функции хелпера для  удаления модального окна из верстки тогда когда это необходимо
*/
