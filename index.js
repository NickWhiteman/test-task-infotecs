...
/* Код */
const tableDiv = document.querySelector(".table-container");/* HTMLElement для таблицы */
const editRow = document.querySelector(".edit-row");/* Тут будет размещаться модальное окно */
const body = document.querySelector(".body");/* Боди для добавления елементов в вертку */
const blackDisplay = document.createElement('div');/* div для затемнения экрана */
/* Получаю доступ к кнопкам сортировок таблицы */
const buttonSortFirstName = document.querySelector(".firstName");
const buttonSortLastName = document.querySelector(".lastName");
const buttonSortFirstNameCBA = document.querySelector(".firstNameCBA");
const buttonSortLastNameCBA = document.querySelector(".lastNameCBA");
const buttonSortAbout = document.querySelector(".about");
const buttonSortEyeColor = document.querySelector(".eye-color");
/* Вешаю слушаетелей клика по кнопкам сортировок и задаю параметры */
buttonSortFirstName.addEventListener('click', () => {sortDataName('firstName', 'abc')});
buttonSortLastName.addEventListener('click', () => {sortDataName('lastName', 'abc')});
buttonSortFirstNameCBA.addEventListener('click', () => {sortDataName('firstName', 'cba')});
buttonSortLastNameCBA.addEventListener('click', () => {sortDataName('lastName', 'cba')});
buttonSortAbout.addEventListener('click', () => {sortAboutLength()});
buttonSortEyeColor.addEventListener('click', () => {sortEyesColor()});
/* Переменные флаги для расчетов отображаемой страницы */
const pageCount = 10; //колличество элементов на странице
let openingPage = 0; //номер открытой страницы
let numberPage = 1; //переменная начала отстчета отображаемых кнопок порядка страниц

/*
  Функция tableRender принимает в себя параментр номера страницы, затем pageTableRow возвращает
  массив строк для таблицы. Затем в Родительский div таблицы выгружаем верстку таблицы.
*/
const tableRender = (pageNumber) => {
  let page = pageTableRow(pageNumber);
  tableDiv.innerHTML = tableCreator(page);
}

/*
  Функция pageTableRow по индексу открытой странице формирует необходимое колличество строк для
  таблицы
*/
const pageTableRow = (pageIndex) => {
  const rows = data.slice(pageIndex * pageCount, (pageIndex + 1) * pageCount);
  return rows;
};

/*
  Сортировки 
    sortAboutLength сортировка по длине строки
    sortDataName сортировка по алфавиту от а до я и наоборот 
    sortEyesColor сортировка по цветам по алфавиту.
*/
const sortAboutLength = () => {
  const sortData = [];
  let aboutCollection = data.map((item) => ({id: item.id, about: item.about}));
  aboutCollection.sort((a, b) => a.about.length > b.about.length ? 1 : -1);
  aboutCollection.forEach((about) => {
    data.forEach((item) =>
      about.id === item.id && sortData.push(item))}
  );

  data = sortData;
  tableRender(openingPage);
};

const sortDataName = (name, sortOrder) => {
  const sortData = [];
  let dataName = data.map(item => item.name[name]);
  sortOrder === 'abc'
  ? dataName.sort()
  : dataName.sort((a, b) => a < b ? 1 : -1);
  dataName.forEach((names) => {
    data.forEach((item) => 
    names === item.name[name] && sortData.push(item))}
  );
...
const sortEyesColor = () => {
  /*Cоздаем массив для хранения сортированных данных*/
  const sortData = []; 
  /*В переменную массив eyesColor возвращаем обьекты с полями из даты id и eyeColor*/
  let eyesColor = data.map(item => ({id: item.id, color: item.eyeColor})); 
  /*Сортируя массив eyesColor прописываем настройку сортировки*/
  eyesColor.sort((a, b) => a.color > b.color ? 1 : -1);
  /*После ищем по id соответствие данных и собираем сортированный массив*/
  eyesColor.forEach((color) => {
    data.forEach((item) => 
      color.id === item.id && sortData.push(item))}
  );
...
/*
  Функция setEdition() вызывает появление модального окна с информацией из строки
*/
const setEdition = (index) => { 
  deleteEditWindow();
  blackDisplay.className = 'displayBlock';
  body.appendChild(blackDisplay);
  const nowOpenPage = pageTableRow(openingPage);
  editRow.innerHTML += modalWindow(nowOpenPage[index]);
};

/* 
  Функция reRenderPage когда была нажата кнопка выбора страницы таблички, занимается тем что
  перерисовывает таблицу в верстке и запоминает в переменной openingPage номер открытой страницы
*/
const reRenderPage = (pageNumber) => {
  tableDiv.innerHTML = '';
  tableRender(pageNumber);
  openingPage = pageNumber;
};

/* 
  Функция pageButtons собирает нужное колличество кнопок для обьектов с данными. для перехода со станички на страничку таблицы. 
  !!! Тут я не отбил случай когда элементов будет чем 50 элементов например 52 и нужно будет отобразить на одной страничке 2 строки. Сейчас это не работает.
*/
const pageButtons = () => {
  const htmlButtons = `
    ${data.map((item, index) => {
      if(index % 10 === 0) {
        const html = `
          <button 
            class='num-page' 
            onclick='reRenderPage(${numberPage-1})'>${ numberPage }</button>`;
        numberPage++
        return html;
      }
    })
...
/*
  Функция deleteEditWindow Занимается тем что удаляет окно редактирования данных строки, 
  в случае если существует. Несет роль функции хелпера для  удаления модального окна из 
  верстки тогда когда это необходимо
*/
const deleteEditWindow = () => {
    let editBlock = document.querySelector(".edition-block");
...
const saveData = () => {
  alert('Извините сохранение не реализовано');
};

/*
  Функция modalWindow рисует модальное окно с данными конкретной строки
*/
const modalWindow = (data) => {
    return(
       `<div class='edition-block'>
            <div class='edit-wrapper'>
                <div class='close-edit' onclick='deleteEditWindow()'>
                    <h3>Edition data</h3>
                    <img class='close' src="https://img.icons8.com/ios/50/000000/multiply.png"/>
                </div>
                <div class='form-field'>
                    <label>FirstName:</label>
                    <input type="text" value="${data.name.firstName}"></input>
                </div>
                <div class='form-field'>
                    <label>LastName:</label>
                    <input type="text" value="${data.name.lastName}"></input>
                </div>
                <div class='form-field'>
                <label>Phone:</label>
                ...
                <div class='form-field'>
                    <label>About:</label>
                    <textarea type="text" placeholder="${data.about}"></textarea>
                </div>
                <div class='form-field'>
                    <label>EyeColor:</label>
                    <input type="text" value="${data.eyeColor}"></input>
                </div>
                <div class='button-footer'>
                    ...
                    <button 
                        class='cancel-button' 
                        type='button'
                        onclick='deleteEditWindow()'>Cancel</button>
                </div>
            </div>
        </div>`
    )
...
/*
  Функция dataMapper собирает верстку строки для отображения в таблице
*/
const dataMapper = (data) => (
    data.map((item, index) => {
        return(
        `<tr class='item-table' onclick='setEdition(${index})'>
            <td class='item'>${item.name.firstName}</td>
            <td class='item'>${item.name.lastName}</td>
            <td class='item about-column'>${item.about}</td>
            <td class='item ${item.eyeColor}'></td>
        </tr>`
    )})
    .join('')
);
...
/*
  Функция tableCreator создает верстку таблицы со своим содержимым и кнопками
*/
const tableCreator = (data) => {
    return (`<table class='table'>
            <tr>
                <th class='header-item'>FirstName</th>
                <th class='header-item'>LastName</th>
                <th class='header-item'>About</th>
                <th class='header-item'>EyeColor</th>
            </tr>
            ${dataMapper(data)}
        </table>
...
/*
  Вызываем функцию tableRender и передаем параметром переменную openingPage для запуска таблицы
*/
tableRender(openingPage);
